...
# sol1 = solve_ivp(funtion_model, [0, -300], [1.409, 0.85, 0.], args=(1.04,), max_step=.1)
...
for i in range(len(Rb_values)):
    ...
    PC['EQ1'].forward()
    PC['EQ1'].backward()
    PC['EQ1'].display(['L','x'], stability=True, figure=1, color=colors[i])
...
# Create bifn curve for limit cycle
    ...
    PC['LC1'].forward()
    PC['LC1'].backward()
    PC['LC1'].display(('L', 'x'), stability=True, figure=1, points=True, color=colors[i])
